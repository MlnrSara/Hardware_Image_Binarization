...
#endif
static int CheckData(int dataSize);
...
XAxiDma AxiDma;
FATFS fileSystem;
TCHAR *path = "0:/"; // path to the SD card
int dataSize;
...
#endif
{
  ...
  openFileResult = f_open(&readFilePointer, "0:/dots.bmp", FA_READ);
  if (openFileResult != FR_OK) {
    xil_printf("ERROR: f_open returned: %d\r\n", openFileResult);
  }

  readSize = readFilePointer.obj.objsize;
  int dataSize = readSize;

  FRESULT readFileResult;
  readFileResult = f_lseek(&readFilePointer, 0);
  if (readFileResult != FR_OK) {
    xil_printf("ERROR: f_lseek returned: %d\r\n", readFileResult);
  }
  unsigned int br;
  readFileResult = f_read(&readFilePointer, TxBufferPtr, readSize, &br);
  if (readFileResult != FR_OK) {
    xil_printf("ERROR: f_read returned: %d\r\n", readFileResult);
  }
  // Close file
  readFileResult = f_close(&readFilePointer);
  if (readFileResult != FR_OK) {
   ...
   * is enabled
   */
  Xil_DCacheFlushRange((UINTPTR)TxBufferPtr, dataSize);
  Xil_DCacheFlushRange((UINTPTR)RxBufferPtr, dataSize);

  for (Index = 0; Index < Tries; Index++) {

    Status = XAxiDma_SimpleTransfer(&AxiDma, (UINTPTR)RxBufferPtr, dataSize,
    ...
    Status = XAxiDma_SimpleTransfer(&AxiDma, (UINTPTR)TxBufferPtr, dataSize,
    ...
    openFileResult =
        f_open(&writeFilePointer, "0:/result.bmp", FA_OPEN_ALWAYS | FA_WRITE);
    if (openFileResult != FR_OK) {
      xil_printf("ERROR: f_open returned: %d\r\n", openFileResult);
    }
    unsigned int bw;
    // write the header
    /*readFileResult = f_write(&writeFilePointer, TxBufferPtr, dataSize, &bw);
    if (readFileResult != FR_OK) {
      xil_printf("ERROR: f_write returned: %d\r\n", readFileResult);
    }*/
    // write the processed data
    readFileResult = f_write(&writeFilePointer, RxBufferPtr, readSize, &bw);
    if (readFileResult != FR_OK) {
      xil_printf("ERROR: f_write returned: %d\r\n", readFileResult);
    }
    // Close file
    readFileResult = f_close(&writeFilePointer);
    if (readFileResult != FR_OK) {
      xil_printf("ERROR: f_close returned: %d\r\n", readFileResult);
    }
...
static int CheckData(int dataSize) {
   ...
   */
  Xil_DCacheInvalidateRange((UINTPTR)RxPacket, dataSize);
